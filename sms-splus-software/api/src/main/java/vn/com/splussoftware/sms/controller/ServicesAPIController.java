...
public class ServicesAPIController {
	@Autowired
	private ServicesRepository repoServices;
	@ Autowired
...
//		}
		...
		if (providerRepo.exists(service.getProviderId())) { // check id of table reference is exist
			ServicesValidator.checkServiceData(errorList, service);// kietlt 9:44 AM  check validate input of data
			...
			validatorErrorModelException.setErrorMessage(ServicesConstant.FAIL_NOT_FOUND_PROVIDER);
		...
		if (errorList.isEmpty()) { // kietlt 3:12 PM 2016/1/27 data of service
			...
			return ServicesConstant.SUCCESS_CREATE;
	...
	public String deleteMul(@RequestBody List<Integer> listId) {
		...
		if (flag) {
			return ServicesConstant.SUCCESS_DEL;
		}
	...
	public String updateId(@RequestBody ServicesEntity serviceEdited) {
		...
		if (repoServices.exists(serviceEdited.getId())) {
			...
			if (errorList.isEmpty()) {
				...
				return ServicesConstant.SUCCESS_UPDATE;
	...
	public String disableId(@PathVariable(value = "id") Integer id) {
		if (repoServices.exists(id)) {
			ServicesEntity serviceData = new ServicesEntity();
			serviceData = repoServices.findOne(id);
